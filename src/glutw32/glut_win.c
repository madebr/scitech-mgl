...
#if 1
/* Added for identifying visual info as string to user. (DaveM) */
static char *
getVisualString(unsigned int mode)
{
  static char str[255];

  sprintf(str, "%s color, %s%s%s%s%s%s buffers",
    GLUT_WIND_IS_RGB(mode) ? "RGB" : "PALETTE",
    GLUT_WIND_IS_SINGLE(mode) ? "SINGLE" : "DOUBLE",
    GLUT_WIND_IS_STEREO(mode) ? "+STEREO" : "",
    GLUT_WIND_HAS_ALPHA(mode) ? "+ALPHA" : "",
    GLUT_WIND_HAS_DEPTH(mode) ? "+DEPTH" : "",
    GLUT_WIND_HAS_STENCIL(mode) ? "+STENCIL" : "",
    GLUT_WIND_HAS_ACCUM(mode) ? "+ACCUM" : "");
  return str;
}
#endif
  ...
  int x, int y, int width, int height, int gameMode)
{
  ...
  if (!window->vis) {
    __glutFatalError(
#if 1 /* Provide pixel format caps summary back to user */
      "visual with necessary capabilities not found (%s).", getVisualString(__glutDisplayMode));
#else
  ...
  if (!window->vis) {
    __glutFatalError(
#if 1 /* Provide pixel format caps summary back to user */
      "pixel format with necessary capabilities not found (%s).", getVisualString(__glutDisplayMode));
#else
