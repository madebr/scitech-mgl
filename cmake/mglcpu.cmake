function(MGL_DetectTargetCPUArchitectures DETECTED_ARCHS)
...
${src_main}
  ...
  set(path_src_arch_detect "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/CMakeTmp/MGL_detect_arch.c")
  file(WRITE "${path_src_arch_detect}" "${src_arch_detect}")
  set(path_dir_arch_detect "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/CMakeTmp/MGL_detect_arch")
  ...
  try_compile(MGL_CPU_CHECK_ALL
    "${CMAKE_CURRENT_BINARY_DIR}/CMakeFiles/CMakeTmp/MGL_detect_arch"
    SOURCES "${path_src_arch_detect}"
    COPY_FILE "${path_bin_arch_detect}"
  )
  cmake_pop_check_state()
  if(NOT MGL_CPU_CHECK_ALL)
  ...
  if(detected_archs)
    foreach(known_arch IN LISTS known_archs)
      set("MGL_CPU_${known_arch}" "${detected_${known_arch}}" CACHE BOOL "Detected architecture ${known_arch}")
    endforeach()
    ...
    foreach(known_arch IN LISTS known_archs)
      if(NOT detected_archs)
        set(cache_variable "MGL_CPU_${known_arch}")
          set(test_src "
        int main(int argc, char *argv[]) {
        #if ${arch_check_${known_arch}}
          return 0;
        ...
        if(${cache_variable})
          set(MGL_CPU_${known_arch} "1" CACHE BOOL "Detected architecture ${known_arch}")
          set(detected_archs ${known_arch})
        else()
          set(MGL_CPU_${known_arch} "0" CACHE BOOL "Detected architecture ${known_arch}")
        endif()
      endif()
    endforeach()
    cmake_pop_check_state()
